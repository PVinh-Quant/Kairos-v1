...
> [!NOTE]
> Module này hiện đang được cung cấp dưới dạng stub (placeholder) trả về dữ liệu mẫu rỗng để tránh lỗi import khi chạy và kiểm thử hệ thống.
...
> [!NOTE]
> Module này hiện đang được cung cấp dưới dạng stub (placeholder) trả về dữ liệu mẫu mặc định để tránh lỗi import khi chạy và kiểm thử hệ thống.
...
Chứa 4 file thực thi độc lập tương ứng với các chế độ chạy của hệ thống (ngoại trừ Dashboard nằm ở thư mục `hien_thi/`), được gọi từ menu launcher chính (`main.py`).
